...
for T = 1:param.num
    ...
    % Subset of size 63% random chosen
    ...
    for n = 1:2^param.depth-1
        if ~isempty(tree(T).node(n).idx)
            ...
            %if leaf node, normalise class distribution at leaf,
            %give node a leaf index, the label of this in trees leaves,
            %gives tree(1).prob(cnt_total,:) location
            if ~tree(T).node(n).dim % if this is a leaf node dim=0
                tree(T).node(n).leaf_idx = cnt;
                tree(T).leaf(cnt).label = cnt_total;
                %auto shapes node label distribution into x by 1 array
                ...
                %get distribution of classes at each leaf
                if strcmp(param.split,'Var')
                    tree(1).cc(cnt_total,:) = mean(data(tree(T).node(n).idx,1:end-1),1); % For RF clustering, unfinished
                else
                    %add class distribution at leaf to total in tree(1) for all trees
                    tree(1).prob(cnt_total,:) = tree(T).node(n).prob';
                end
                
                cnt = cnt+1;%for this tree
                cnt_total = cnt_total + 1;%for all trees
            end
        end
    end
end
