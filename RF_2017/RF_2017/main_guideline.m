...
for bag= 1:4
    ...
    training_bags= data_train(rand_select(1, 1:bag_size).',:);
    plot_toydata(training_bags);
    figure;
end
...
ð?œ™ð?œ™(v)Â·ð?œ“ð?œ“,for splitting data v.
â€“
...
h(v,ð?œƒð?‘—) ={0 or1}
â€“
ð?œƒð?‘— âˆˆ denote the parameters of the test function at the j-thsplit node.
â€“
The data point
...
The filter function
ð?œ™ selects some features of choice out of the entire vector v.
â€“
ð?œ“ defines the geometric primitive used to separate the data (e.g., an axis-aligned hyperplane, an oblique hyperplane, a general surface etc.)
â€“
The parameter ð?œ? is a threshold for the inequalities used in the binary test.

parametrization of the weak learner model as
Î¸=(ð?œ™,ð?œ“,ð?œ?). Learn function based on incoming data subset that best splits the data. maximisation objective function to split node data, max info gain
...
param.splitNum = 3;     % Number of split functions to try
splitopts={'twopixel'};%,'kmean' need to sort out kmean, use cluster centers , 'twopixel' 'IG', 
depthopts=[10];%[3,5,10,12];
numopts=[12];%[3,5,10,12];
splitnumopts=[5];%[1,3,5];
length(splitopts)

for num=1:length(numopts)
    for depth=1:length(depthopts)
        for splitnum=1:length(splitnumopts)
            for i=1:length(splitopts)
                param.split = splitopts(i);     % Currently support 'information gain' only twopixel kmean
                param.depth= depthopts(depth);
                param.splitNum=splitnumopts(splitnum);
                param.num=numopts(num)
                tic
                trees = growTrees(data_train,param);
                toc
            end
...
% Test on the dense 2D grid data, and visualise the results ... 
    ...
    for vote =1:length(voting_scheme)
        ...
        for m=1:size(data_test,1)
            ...
            p_rf = trees(1).prob(leaves,:); %10 trees in forest, 10 leaf distributions.
            if ( strcmp(voting_scheme(vote), 'sum-avg'))
                p_rf_sum = sum(p_rf)/length(trees);
                [val, idx] = max(p_rf_sum);
                data_test(m,3)=idx;
            end
            if ( strcmp(voting_scheme(vote), 'product'))
                p_rf_sum = prod(p_rf,1)/length(trees);
                [val, idx] = max(p_rf_sum);
                data_test(m,3)=idx;
            end
            if ( strcmp(voting_scheme(vote), 'maj-vote'))
                total=[0,0,0];
                for i = 1:size(p_rf,1)
                    [val,idx]=max(p_rf(i,:));
